...
ui <- navbarPage(title = "Citibank",
                              ...
                              mainPanel(
                                  tabsetPanel(type = "tabs",
                                              tabPanel("Map", 
                                                       ...
                                                       br(),
                                                       br(),
                                                       span("Which market can we potentially grow? It is these 5 countries with high average ticket because we know they are high spenders. We can further market tourists from these countries to increase awareness, thus increasing the number of high-ticket transactions from them."),
                                                       br(),
                                                       br(),
                                                       ...
                                                       br(),
                                                       br(),
                                                       span("When 'Country' is selected, we drill down into the top categories in terms of spending and noticed that Fashion & Shoes highly attracted the Chinese tourists. An international campaign targeted the Asian maat rket can be done as there is an appetite from Japan & China’s tourists into the fashion industry. Also, US’s tourists show powerful appetite for Bars & Restaurants category, this can be a sign for having US’s themed restaurants & bars can be furthered invested in to attract more US tourists."),
                                                       br(),
                                              ...
                                              tabPanel("Distribution", 
                                                       ...
                                                       br(),
                                                       br(),
                                                       span("When 'Category' is selected. This ridgelines chart explains the spending distribution by category. The top 3 categories with high variation are Fashion & Shoes, Accommodation, Bars & Restaurants. These 3 categories account for 83% of all spending. Therefore, the focus of promoting places for tourists should be allocated to these categories."),
                                                       br(),
                                              ...
                                              tabPanel("Hour of the day", 
                                                       br(),
                                                       span('We can see similar pattern in purchase behavior between Fridays & Thursdays with slightly higher purchases on Fridays. However, some nationalities are morning spending more than the others. Among the top 5 nationalities in terms of purchases (USA, United Kingdom, China, France and Japan), tourists from US & Japan exhibited early spending pattern and can be an opportunity to target these tourists with customized offers.'),
                                                       br(),
                                                       plotOutput('plot_distribution_hourofday'),
                                                       br(),
                                                       br(),
                                                       span("When 'Category' is selected. This visualization is a heatmap that explains the purchase behavior for all categories by the hour of the day. Here we can see noticeable variation based on the purchase category, for example the peak hours of the category Fashion & Shoes begin in the afternoon (16h00) and starts to decline after 18h00. Our recommendation is to activate marketing campaigns during the off-peak hours in specific categories (i.e., Bars & Restaurants) to encourage tourists visiting these establishments with tempting discounts or offers."),
                                                       br(),
                 ...
                 tabPanel("Growth",
                          sidebarLayout(
                              ...
                              mainPanel(
                                  span("'Growth Frontier' Segment - the segment that needs to be nurtured as this is where the largest volume of aggregated transactions and sales comes from. We can execute campaigns that help boost further both the number of transactions and the ticket value through a cross sell upsell program. Part of nurturing is making them come back. To do this, a loyalty program can be designed."),
                                  br(),
                                  br(),
                                  span("'Spend More' Segment - the segment that contains the three countries with extremely higher number of transactions but with relatively lower ticket value. The opportunity we can see here is let them spend more. An in-store product promotion can be performed to entice the customers spend more as they realize value for money."),
                                  br(),
                                  br(),
                                  span("'More Transaction' Segment - the segment that has countries with higher ticket value but lesser number of transactions. A campaign that involve cross-store tie ups can make the customers shop more increasing the number of transactions they make. Another way to look at it is that the low number of transactions can be a function of low number of tourists coming from these countries that are geographically far from Spain. Awareness campaign and airline tie-ups can be done to entice more number of tourists come visit Spain."),
                                  br(),
                                  br(),
                                  br(),
